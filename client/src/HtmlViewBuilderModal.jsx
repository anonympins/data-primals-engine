    ...
    return (
        ...
        >
            <div className="form-grid">
                ...
                <CodeField name="filter" label={t('filter', 'Filtre (JSON)')} value={typeof config.filter === 'string' ? config.filter : JSON.stringify(config.filter || {}, null, 2)} language="json" onChange={(value) => { try { const parsed = JSON.parse(value); handleCodeChange('filter', parsed); } catch (e) { /* Gérer erreur */ } }} />
                <CodeField name="template" label={t('dashboards.htmlTemplate', 'Template HTML')} value={config.template || ''} language="html" onChange={(value) => handleCodeChange('template', value)} required height="200px" />
                <CodeField name="css" label={t('dashboards.cssStyles', 'Styles CSS')} value={config.css || ''} language="css" onChange={(value) => handleCodeChange('css', value)} height="150px" />
            </div>

...
## 🛠️ Example: Advanced Data Model

Data Primal Engine allows you to define complex, nested structures with advanced database features. Here is a comprehensive example of a **Corporate Profile** model:

```json
{
    "name": "corporate_profile",
    "description": "Comprehensive model for multi-branch organization management.",
    "icon": "FaBuilding",
    "fields": [
        { "name": "legalName", "type": "string", "required": true, "asMain": true },
        { "name": "bio", "type": "richtext_t", "hint": "Multilingual company biography" },
        {
            "name": "structure",
            "type": "object",
            "fields": [
                ...
                { 
                    ...
                    "type": "object", 
                    "fields": [
                        { "name": "taxId", "type": "string", "anonymized": true, "hint": "Auto-anonymized for non-admin users" },
                        { "name": "bankVaultKey", "type": "string", "encrypted": true }
                    ]
                }
            ]
        },
        {
            ...
            "itemsType": "object",
            "fields": [
                { "name": "city", "type": "string", "index": true },
                { "name": "coordinates", "type": "geolocation", "index": true, "indexType": "2dsphere" }
            ]
        },
        { "name": "tags", "type": "array", "itemsType": "string", "index": true, "indexType": "text" },
        { "name": "category", "type": "enum", "items": ["Startup", "SMB", "Enterprise"], "default": "SMB" }
    ],
    "constraints": [
        { 
            "name": "unique_org_per_user", 
            "type": "unique", 
            "keys": ["legalName", "_user"] 
        }
    ]
}
```
...
*   **Recursive Objects**: The `structure` field contains a nested `compliance` object. The engine handles validation, anonymization, and encryption at any depth.
*   **Advanced Indexing**: 
    *   `2dsphere` on `branches.coordinates` enables proximity searches (e.g., "Find branches near me").
    *   `text` index on `tags` allows high-performance keyword searching across documents.
*   **Security**: The `taxId` is marked as `anonymized`, meaning it will be obfuscated for standard users while remaining visible to administrators.
*   **Integrity**: A composite unique constraint ensures that a user cannot create two organizations with the same `legalName`.

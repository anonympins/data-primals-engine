            ...
            if (existingModel) {
                ...
                if( count < maxModelsPerUser) {
                    if(await engine.userProvider.hasFeature(req.me, 'indexes')){
                        ...
                        // Handle composite unique indexes from constraints
                        if (modelData.constraints) {
                            for (const constraint of modelData.constraints) {
                                if (constraint.type === 'unique' && Array.isArray(constraint.keys) && constraint.keys.length > 0) {
                                    const indexKeys = {};
                                    constraint.keys.forEach(key => {
                                        indexKeys[key] = 1;
                                    });
                                    await datasCollection.createIndex(indexKeys, {
                                        unique: true,
                                        partialFilterExpression: {
                                            _model: modelData.name,
                                            _user: req.me.username
